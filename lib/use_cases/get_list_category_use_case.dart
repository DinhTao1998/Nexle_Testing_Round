...
@lazySingleton
class GetListCategoryUseCase extends BaseUseCase<GetListCategoryUseCaseInput,GetListCategoryUseCaseOutput> with LogMixin{
  @override
  Future<GetListCategoryUseCaseOutput> buildUseCase([GetListCategoryUseCaseInput ?input]) async {
    try{
      DataListResponse<CategoryData> data = await AppApiService.getInstance().getCategories(logService: LogServiceWithToast(context: Get.context!));
      if(data.data != null) {
        List<CategoryModel> categoryModel = GetIt.instance.get<CategoryDataMapper>().mapToListEntity(
            data.data);
        return GetListCategoryUseCaseOutput(categoryModel);
      }else{
        return const GetListCategoryUseCaseOutput ([]);
      }
    }
    catch(e){
      return const GetListCategoryUseCaseOutput([]);
...
class GetListCategoryUseCaseInput extends BaseUseCaseInput{}
class GetListCategoryUseCaseOutput extends BaseUseCaseOutput{
  const GetListCategoryUseCaseOutput(this.listModel);
